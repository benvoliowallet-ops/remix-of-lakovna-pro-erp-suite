...
export default function OrderProtocolAcceptance() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const [showPrices, setShowPrices] = useState(searchParams.get('ceny') === 'true');

  const { data: order, isLoading } = useQuery({
    queryKey: ['order-protocol', id],
    queryFn: async () => {
  ...
  const sortedItems = [...(order.order_items || [])].sort((a, b) => {
    ...
    // Base coat (zaklad) before top coat
  ...
  const vatAmount = order.company?.is_vat_payer ? totalPrice * 0.23 : 0;
        ...
        <div className="mx-auto max-w-4xl flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="show-prices">Zobraziť ceny</Label>
            </div>
        ...
        <div className="protocol-a4 bg-white p-8 shadow-sm print:shadow-none">
          ...
          <div className="border-b pb-6 mb-6">
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-2xl font-bold text-foreground">PRIJÍMACÍ PROTOKOL</h1>
                <p className="text-muted-foreground mt-1">
                  Zákazka č. {order.id}
                </p>
                <div className="mt-3">
                  <QRCodeSVG
                    value={`https://lakovna-pro-smart.lovable.app/status/${order.id}`}
                    size={80}
                  />
                  <p className="text-xs text-muted-foreground mt-1">Sledovanie zákazky</p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 gap-8 mb-8">
            ...
            <div>
              ...
              {order.deadline_at && (
                <p><span className="text-muted-foreground">Termín:</span> {new Date(order.deadline_at).toLocaleDateString('sk-SK')}</p>
              )}
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b-2 border-foreground">
                  ...
                  {showPrices && <th className="py-2 text-right font-semibold">Cena</th>}
                </tr>
              </thead>
              <tbody>
          ...
          {showPrices && (
            <div className="mb-8 flex justify-end">
              <div className="w-64">
                ...
                <div className="flex justify-between py-2 border-t-2 border-foreground font-bold">
                  <span>Celkom:</span>
          ...
          <div className="mt-12 pt-8 border-t">
            <div className="grid grid-cols-2 gap-8">
              <div>
                <p className="text-sm text-muted-foreground mb-12">Podpis zákazníka:</p>
                <div className="border-b border-foreground"></div>
              </div>
              <div>
                <p className="text-sm text-muted-foreground mb-12">Dátum prevzatia:</p>
                <div className="border-b border-foreground"></div>
              </div>
            </div>
          </div>
